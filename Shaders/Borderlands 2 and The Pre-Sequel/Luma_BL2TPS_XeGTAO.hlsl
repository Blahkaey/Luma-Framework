// XeGTAO implementation adapted for Borderlands 2 / The Pre-Sequel.
//
// Reference: https://github.com/GameTechDev/XeGTAO

cbuffer GamePSConstants : register(b0)
{
   float4 psc[18]; // [16].zw = texel size, [17].z = depth scale
}

cbuffer GameVSConstants : register(b1)
{
   float4 vsc[31]; // [29].x = projection m00, [30].y = projection m11
}

cbuffer LumaXeGTAO : register(b2)
{
   float4 xe_params0; // intensity, radius near (screen UV), radius far (screen UV), falloff range
   float4 xe_params1; // distribution power, thin occluder compensation, final power, radius near->far lerp distance (view units)
}
...
// The vanilla SSAO's sampling footprint (SSAO.usf) is a screen-UV radius: near value up close, lerping to
// the far value over the first EFFECT_RADIUS_LERP_DIST view units (~1m). Its AODistScale=0.15 is NOT the
// radius — it only scales the (near-flat) distance attenuation, cutting off at footprint/0.15.
#define EFFECT_RADIUS_NEAR_UV      xe_params0.y
#define EFFECT_RADIUS_FAR_UV       xe_params0.z
#define EFFECT_FALLOFF_RANGE       xe_params0.w
#define SAMPLE_DISTRIBUTION_POWER  xe_params1.x
#define THIN_OCCLUDER_COMPENSATION xe_params1.y
#define FINAL_VALUE_POWER          xe_params1.z
#define EFFECT_RADIUS_LERP_DIST    xe_params1.w
...
// Matches the game's ScreenPosScaleBias interpolant.
...
// Saturated depth is the game's no-data value; keep it outside the AO radius without overflowing fp32.
#define XE_GTAO_SATURATED_DEPTH 0.999
#define XE_GTAO_DEPTH_SENTINEL  1e16
...
// Vanilla sampling footprint -> XeGTAO world-space radius. The .usf footprint is a UV fraction; the true
// horizontal world extent at depth z is d(view.x)/d(uv.x) * uv = NDC_TO_VIEW_MUL.x * z * uv (vanilla's own
// "radiusInViewSpace = uv * z" skips the projection scale, but it only feeds its near-flat attenuation).
// Safe with XE_GTAO_DEPTH_SENTINEL: a huge z yields a huge radius, like the old depth-proportional form.
float XeGTAO_EffectRadius(float viewspaceZ)
{
   float uvRadius = lerp(EFFECT_RADIUS_NEAR_UV, EFFECT_RADIUS_FAR_UV, saturate(viewspaceZ / EFFECT_RADIUS_LERP_DIST));
   return uvRadius * NDC_TO_VIEW_MUL.x * viewspaceZ * RADIUS_MULTIPLIER;
}
...
void XeGTAO_MainPass(uint2 pixCoord, float2 localNoise, float3 viewspaceNormal, Texture2D sourceViewspaceDepth, SamplerState depthSampler, RWTexture2D<unorm float2> outWorkingAOTermAndEdges)
{
   ...
   // Isolate saturated depth from the denoiser.
   ...
   // see "Algorithm 1" in https://www.activision.com/cdn/research/Practical_Real_Time_Strategies_for_Accurate_Indirect_Occlusion_NEW%20VERSION_COLOR.pdf
   {
      ...
      visibility = pow(max(0.0, visibility), FINAL_VALUE_POWER); // guard against negative approximation error
...
#if XE_GTAO_FINAL_APPLY
RWTexture2D<float2> final_output : register(u0);
#else
