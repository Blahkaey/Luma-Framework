// XeGTAO implementation adopted for Borderlands 2 / The Pre-Sequel (UE3 via dgVoodoo2 2.81.3).
//
// For the reference see: https://github.com/GameTechDev/XeGTAO
// Adapted from the Deus Ex: Human Revolution Luma port (the depth-derived-normals variant; forward-rendered
// UE3 exposes no normal buffer).
//
// Replaces the game's SSAO occlusion pass (PS 0x09C9AD0F): input is the game's own AO depth buffer ("buffer A",
// R32G32F (d, d), d = linear view depth normalized by 1/MaxOcclusionDepth, sky saturated to 1 by the depth-setup
// pass 0x48CA4E61), and the final denoise writes the game's AO buffer convention (AO, d) so the vanilla apply
// pass (0x89122ECA: discard at AO >= 0.995, OcclusionColor tint by INV_SRC_ALPHA blend) composites it unchanged.
// The dispatches inherit the hijacked draw's own game constants (dgVoodoo keeps SEPARATE per-stage cb4 buffers,
// both bound by C++, see main.cpp RunXeGTAO).

cbuffer GamePSConstants : register(b0) // dgVoodoo PS cb4 of the occlusion draw
{
   float4 psc[18]; // [16].zw = AO buffer texel size (1/w, 1/h), [17].z = depth scale (view depth = d * this)
}

cbuffer GameVSConstants : register(b1) // dgVoodoo VS cb4 of the occlusion draw
{
   float4 vsc[31]; // [29].x = projection m00, [30].y = projection m11 (cot half FoV per axis, live per frame)
}

cbuffer LumaXeGTAO : register(b2) // fixed AO tuning plus live intensity, uploaded by main.cpp RunXeGTAO
{
   float4 xe_params0; // x = intensity (1 = neutral), y = radius depth fraction, z = fixed radius (view units), w = falloff range
   float4 xe_params1; // x = sample distribution power, y = thin occluder compensation, z = final value power, w = unused
}

// Generate viewspace normals from depth (no G-buffer in forward UE3)
#define XE_GTAO_GENERATE_NORMALS 1

// Fallback in case the global define is missing (normally provided by Luma's Advanced Settings)
...
// Runtime parameters (only intensity is user configurable; the remaining values are fixed in main.cpp)
//
// The AO radius is DEPTH-PROPORTIONAL (radius = EFFECT_RADIUS_DEPTH_FRACTION * viewspaceZ + EFFECT_RADIUS),
// like the game's own SSAO (~0.13-0.27 * z per its disassembly), instead of XeGTAO's stock fixed viewspace
// radius: the view-unit scale (psc[17].z) is large and unknown, so any fixed radius collapses to a sub-pixel
// screen radius at scene depths (no AO anywhere except very near geometry, e.g. the first person weapon).
// A proportional radius cancels the unit scale entirely (the screen-space sampling extent becomes constant
// at every depth, like the vanilla effect). Main visual tuning knob.
#define EFFECT_RADIUS_DEPTH_FRACTION xe_params0.y // Depth-proportional radius term (the game's own SSAO uses ~0.13-0.27)
#define EFFECT_RADIUS                xe_params0.z // Fixed view-unit radius term (keeps close-up AO from shrinking away; 0 = purely proportional)
#define EFFECT_FALLOFF_RANGE         xe_params0.w // Fraction of the radius over which occluders fade out (XeGTAO default 0.615; tiny values pop with a hard edge at the radius boundary)
#define SAMPLE_DISTRIBUTION_POWER    xe_params1.x // Concentrates samples towards the center pixel (XeGTAO default 2.0)
#define THIN_OCCLUDER_COMPENSATION   xe_params1.y // Reduces over-darkening behind thin objects, e.g. the first person weapon (XeGTAO default 0.0)
#define FINAL_VALUE_POWER            xe_params1.z // Occlusion contrast curve (XeGTAO default 2.2)
...
#define VIEWPORT_PIXEL_SIZE psc[16].zw

// The game's own AO vertex shader (0x83EFDA9B) hands its pixel shader exactly this UV -> viewspace mapping as
// the o7 "ScreenPosScaleBias" interpolant: xy = (2u-1)/m00, (1-2v)/m11, scaled by view depth.
#define NDC_TO_VIEW_MUL              float2(2.0 / vsc[29].x, -2.0 / vsc[30].y)
#define NDC_TO_VIEW_ADD              float2(-1.0 / vsc[29].x, 1.0 / vsc[30].y)
...
// The game's depth-setup pass saturates buffer-A depth at 1.0 (= MaxOcclusionDepth = psc[17].z view units), so
// everything beyond that distance reads as a fake fronto-parallel wall standing at exactly that depth. Without
// special handling the wall occludes the band of real geometry just inside the boundary, drawing a hard AO line
// that sweeps with the camera (horizontal across open ground, vertical on side walls). Treat saturated depths as
// "no data": classify them ONCE at the depth-conversion choke point below (all passes read the prefiltered
// pyramid built from it) and map them to a huge finite sentinel depth. The standard falloff/edge math then
// rejects the wall everywhere on its own - sentinel samples sit far outside any radius (weight 0 at every MIP,
// including filtered ones: XeGTAO_DepthMIPFilter's weighting collapses any quad containing a sentinel to the
// sentinel), and edges toward sentinel neighbors compute to 0, isolating the region in the denoiser. Sentinel
// pixels themselves early-out with AO 1 (the apply pass discards at >= 0.995), restored after the final blur
// with the same XE_GTAO_SATURATED_DEPTH threshold so no pixel can be skipped-but-not-restored.
#define XE_GTAO_SATURATED_DEPTH 0.999 // raw buffer-A depth at/above this = no data (shared by classify + restore)
#define XE_GTAO_DEPTH_SENTINEL  1e16  // "no data" viewspace depth; huge vs real depths, but its square stays finite in fp32

float XeGTAO_ScreenSpaceToViewSpaceDepth(const float screenDepth)
{
   // Buffer A depth is already linear (normalized view Z), the game's own occlusion pass applies the same scale
   return screenDepth >= XE_GTAO_SATURATED_DEPTH ? XE_GTAO_DEPTH_SENTINEL : screenDepth * psc[17].z;
}
...
float XeGTAO_DepthMIPFilter(float depth0, float depth1, float depth2, float depth3)
{
   ...
   const float effectRadius = depthRangeScaleFactor * (EFFECT_RADIUS + EFFECT_RADIUS_DEPTH_FRACTION * maxDepth) * RADIUS_MULTIPLIER;
...
void XeGTAO_MainPass(uint2 pixCoord, float2 localNoise, float3 viewspaceNormal, Texture2D sourceViewspaceDepth, SamplerState depthSampler, RWTexture2D<unorm float2> outWorkingAOTermAndEdges)
{
   ...
   // Depth-saturated pixel (beyond MaxOcclusionDepth, see XE_GTAO_DEPTH_SENTINEL): no real geometry here.
   // Zero edges so the denoiser leaves the region fully isolated in both directions.
   [branch] if (viewspaceZ >= XE_GTAO_DEPTH_SENTINEL * 0.5)
   {
   ...
   const float effectRadius = (EFFECT_RADIUS + EFFECT_RADIUS_DEPTH_FRACTION * viewspaceZ) * RADIUS_MULTIPLIER;
   ...
   // see "Algorithm 1" in https://www.activision.com/cdn/research/Practical_Real_Time_Strategies_for_Accurate_Indirect_Occlusion_NEW%20VERSION_COLOR.pdf
   {
      ...
      visibility = pow(max(0.0, visibility), FINAL_VALUE_POWER); // the horizon approximations can leave visibility slightly negative (pow would return NaN, warning X3571)
...
void XeGTAO_Denoise(uint2 pixCoordBase, Texture2D sourceAOTermAndEdges, Texture2D sourceRawDepth, SamplerState texSampler,
...
#if XE_GTAO_FINAL_APPLY
      // Write the game's AO buffer convention: (AO, raw normalized depth). AO 1 = unoccluded, matching both
      // XeGTAO's visibility term and the apply pass's Src*INV_SRC_ALPHA + Dest*SRC_ALPHA blend.
      float rawDepth = sourceRawDepth.Load(int3(pixCoord, 0)).y;
      float ao = saturate(aoTerm[side] * XE_GTAO_OCCLUSION_TERM_SCALE);
      ao = saturate(lerp(1.0, ao, xe_params0.x)); // user AO intensity (1 = neutral)
      if (rawDepth >= XE_GTAO_SATURATED_DEPTH)
      {
         ao = 1.0; // no data (depth saturated, same threshold the main pass skipped at): the apply pass discards at AO >= 0.995
      }
      outputTexture[pixCoord] = float2(ao, rawDepth);
#else
...
#if XE_GTAO_FINAL_APPLY
RWTexture2D<float2> final_output : register(u0); // the (AO, d) copy source for the game's R32G32F AO buffer
#else
...
[numthreads(8, 8, 1)] void prefilter_depths16x16_cs(uint2 dtid : SV_DispatchThreadID, uint2 gtid : SV_GroupThreadID) {
   // tex0 = game AO depth "buffer A" (R32G32F (d, d))
   // smp = point clamp
...
{
   // tex0 = g_srcWorkingDepth (the prefiltered viewspace depth MIP pyramid)
   // smp = point clamp
   // Normals are derived from depth in XeGTAO_MainPass (XE_GTAO_GENERATE_NORMALS).
   XeGTAO_MainPass(dtid, SpatioTemporalNoise(dtid, 0), float3(0.0, 0.0, 0.0), tex0, smp, ao_term_and_edges);
}
...
[numthreads(XE_GTAO_NUMTHREADS_X, XE_GTAO_NUMTHREADS_Y, 1)] void denoise_pass_cs(uint2 dtid : SV_DispatchThreadID) {
   ...
   // tex1 = game AO depth "buffer A" (only bound and read on the XE_GTAO_FINAL_APPLY pass)
   // smp = point clamp
   const uint2 pix_coord_base = dtid * uint2(2, 1); // we're computing 2 horizontal pixels at a time (performance optimization)
   XeGTAO_Denoise(pix_coord_base, tex0, tex1, smp, final_output);
}
